...
const App: React.FC = () => {
  ...
  const handleAllPassagesSubmit = async (finalPassages: string[]) => {
    ...
    try {
      ...
      const numberOfCloseOptionQuestions = Math.round(totalExpectedQuestions * 0.4); // Approx 40%

      const questionsPrompt = `
You are an expert AI assistant specializing in creating exceptionally challenging verbal reasoning questions for competitive entrance exams like CAT (Common Admission Test) and GMAT.
Your generated questions should mirror the complexity and analytical depth of the 90th percentile and above difficulty questions found in these exams.

Based on the provided passage(s), generate EXACTLY ${totalExpectedQuestions} multiple-choice questions (MCQs).
These questions should rigorously test critical reading, nuanced understanding, and advanced analytical skills.
...
- Inference: Questions that require deducing information not explicitly stated, often testing subtle implications.
- Supporting Idea / Specific Detail: Questions about explicitly stated facts or arguments, but potentially requiring careful synthesis of information from different parts of the passage.
- Application: Questions that ask to apply information or principles from the passage to new, complex hypothetical situations.
- Logical Structure / Organization: Questions about how the passage is constructed, the relationship between parts, or the function of specific sentences/paragraphs.
- Author's Tone / Attitude / Style: Questions about the author's perspective, underlying assumptions, or writing style, often requiring attention to subtle cues.
- Vocabulary-in-Context: Questions about the meaning of specific words or phrases as used in the passage, particularly words with multiple meanings or idiomatic expressions.
- Weaken/Strengthen: If a passage presents an argument, include questions that ask to weaken or strengthen it, requiring an understanding of logical fallacies or argumentative structures.
- Identify Assumption: Questions that ask to identify unstated assumptions underlying an argument or statement in the passage.
- Paradox Resolution: If a passage presents an apparent contradiction, questions might ask to resolve it.

The overall difficulty should be very high, targeting aspirants aiming for the highest scores in top-tier MBA programs.
Distractor options (incorrect answers) must be highly plausible, sophisticated, and designed to mislead even test-takers with a strong understanding of the passage. Avoid simplistic or obviously wrong distractors.

CRITICAL INSTRUCTION FOR DISTRACTOR DESIGN:
For EXACTLY ${numberOfCloseOptionQuestions} of the ${totalExpectedQuestions} questions, the distractor options must be *exceptionally* competitive and closely related. These options should:
- Be subtly distinct from the correct answer, perhaps differing by only a key word or phrase that changes the meaning significantly in context.
- Exploit common misinterpretations, over-generalizations, or superficial readings of the passage.
- Test fine-grained understanding of nuances, implications, authorial intent, or the scope of claims made in the passage.
- Be tempting choices for candidates who have a good but not complete or precise grasp of the passage details or its deeper meaning.
The goal for these ${numberOfCloseOptionQuestions} questions is to make the choice extremely challenging, forcing test-takers to re-read relevant parts of the passage carefully and think critically.
(You should internally decide which questions get these close options; do not label them differently in the output JSON.)

For the remaining ${totalExpectedQuestions - numberOfCloseOptionQuestions} questions, maintain a very high difficulty level with sophisticated and plausible distractors as generally required, ensuring they are not trivially easy.

Correct answers should be unambiguously supported by the passage text or logical inference from it, even when options are very close. The justification for the correct answer should be solid and defensible based *only* on the provided passage(s).
...
EXAMPLE OF JSON STRUCTURE (IF ${totalExpectedQuestions} IS 2):
[
  {
    "questionText": "Highly complex sample question text for question 1...",
    "options": ["Very close option A for Q1", "Correct but subtly different option B for Q1", "Plausible distractor C for Q1", "Another sophisticated distractor D for Q1"],
    "correctAnswerText": "Correct but subtly different option B for Q1",
    "explanation": "Detailed explanation for why Option B is correct for Q1, highlighting the subtle distinctions from other options and direct evidence from the passage...",
    "difficultyAssessment": "CAT-Very Hard (90-99th percentile)",
    "commonPitfalls": "Common pitfall for Q1 (e.g., 'Overlooking a specific qualifier in the passage that makes option A incorrect', 'Choosing an option that is true but not the answer to *this* specific question')."
  },
  {
    "questionText": "Challenging sample question text for question 2...",
    ...
    "difficultyAssessment": "GMAT 750+ level",
    "commonPitfalls": "Common pitfall for Q2 (e.g., 'Misinterpreting the scope of the author's claim')."
  }
]
...
{
  "questionText": "A non-empty, clear, and concise question that tests higher-order thinking skills related to the passage(s).",
  ...
  "explanation": "A non-empty, detailed explanation of why the correct answer is correct and why other options (especially plausible distractors) are incorrect, relating directly and specifically to the passage(s).",
  "difficultyAssessment": "A non-empty assessment (e.g., 'CAT-Very Hard (90-99th percentile)', 'GMAT 750+ level'). Be specific about target exam and high difficulty level.",
  "commonPitfalls": "Non-empty, specific reasons students might get this question wrong, focusing on sophisticated CAT/GMAT traps (e.g., 'Misinterpreting complex sentence structure', 'Confusing correlation with causation based on passage information', 'Selecting an option that is too broad or too narrow')."
}
...
5.  "difficultyAssessment": Must be a non-empty string reflecting high difficulty.
...
${combined}
---
Final reminder: Your output must be *only* the valid JSON array as described. Pay special attention to crafting the ${numberOfCloseOptionQuestions} questions with exceptionally competitive and closely related distractors. Double-check all quotes, commas, and brackets.
`;
      ...
      const questionsResponse: GenerateContentResponse = await ai.models.generateContent({
        ...
        config: { 
            ...
            systemInstruction: "You are an expert test designer for competitive exams like CAT and GMAT. Your primary goal is to create challenging and insightful multiple-choice questions based on provided reading comprehension passages, adhering strictly to the user's formatting and content requirements, including specific instructions on distractor difficulty and closeness for a subset of questions."
        }
      });
